...
if not op.isfile(op.join(out_dir, "group_bold.tsv")):
    ...
    cmd = "singularity run --cleanenv \
               ...
               group --no-sub --task-id SST".format(
...
for qc_metric in qc_metrics:
    ...
    if qc_metric in ["efc", "fd_mean", "gsr_x", "gsr_y"]:
        runs_exclude_df = runs_exclude_df.append(
            mriqc_group_SST_df.loc[mriqc_group_SST_df[qc_metric].values > q95], ignore_index=True
        )
    elif qc_metric in ["snr", "tsnr"]:
        runs_exclude_df = runs_exclude_df.append(
            mriqc_group_SST_df.loc[mriqc_group_SST_df[qc_metric].values < q05], ignore_index=True
        )
...
SST_masks = sorted(
    glob(
        op.join(
            bids_dir, "derivatives", derivative, "sub-*", "ses-*", "SST", "*SST*desc-mask.nii.gz"
        )
    )
)
SST_briks = sorted(
    glob(op.join(bids_dir, "derivatives", derivative, "sub-*", "ses-*", "SST", "*SST*tlrc*HEAD"))
)
    ...
    if len(tmp_mask_fn_list) > 1:
        final_mask_list.append(
            op.join(
                ...
                ses,
                "SST",
                "{sub}_{ses}_task-SST_run-1+2_space-MNI152NLin2009cAsym_desc-mask.nii.gz".format(
                    sub=sub, ses=ses
                ),
            )
        )
    ...
    if len(tmp_brik_fn_list) > 1:
        final_brik_list.append(
            op.join(
                ...
                ses,
                "SST",
                "{sub}_{ses}_task-SST_run-1+2_space-MNI152NLin2009cAsym_desc-REMLbucket+SPMG+tlrc.HEAD".format(
                    sub=sub, ses=ses
                ),
            )
        )
...
label_bucket_dict = {
    "correct_stop_gt_correct_go": 21,
    "incorrect_stop_gt_correct_go": 24,
    "correct_stop_gt_incorrect_stop": 27,
}
