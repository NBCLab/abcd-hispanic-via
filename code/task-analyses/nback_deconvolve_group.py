...
if not op.isfile(op.join(out_dir, 'group_bold.tsv')):
    ...
    cmd = 'singularity run --cleanenv \
               ...
               group --no-sub --task-id nback'.format(mriqc_image=mriqc_image,
                                                    bids_dir=bids_dir,
                                                    mriqc_dir=mriqc_dir)
    os.system(cmd)
...
for qc_metric in qc_metrics:
    ...
    if qc_metric in ['efc', 'fd_mean', 'gsr_x', 'gsr_y']:
        runs_exclude_df = runs_exclude_df.append(mriqc_group_nback_df.loc[mriqc_group_nback_df[qc_metric].values > q99], ignore_index=True)
    elif qc_metric in ['snr', 'tsnr']:
        runs_exclude_df = runs_exclude_df.append(mriqc_group_nback_df.loc[mriqc_group_nback_df[qc_metric].values < q01], ignore_index=True)
...
#make a mask
nback_masks = sorted(glob(op.join(bids_dir, 'derivatives', derivative, 'sub-*', 'ses-*', 'nback', '*nback*desc-mask.nii.gz')))
nback_briks = sorted(glob(op.join(bids_dir, 'derivatives', derivative, 'sub-*', 'ses-*', 'nback', '*nback*tlrc*HEAD')))
...
for i, row in runs_exclude_df.iterrows():
    ...
    tmp_run_mask_fn = op.join(bids_dir, 'derivatives', derivative, sub, ses, 'nback', '{}_space-MNI152NLin2009cAsym_res-2_desc-mask.nii.gz'.format(tmp_run))
    if tmp_run_mask_fn in nback_masks:
        nback_masks.remove(tmp_run_mask_fn)

    tmp_run_brik_fn = op.join(bids_dir, 'derivatives', derivative, sub, ses, 'nback', '{}_space-MNI152NLin2009cAsym_res-2_desc-REMLbucket+SPMG+tlrc.HEAD'.format(tmp_run))
...
for tmp_run_mask_fn in nback_masks:
    ...
    if len(tmp_mask_fn_list) > 1:
        final_mask_list.append(op.join(bids_dir, 'derivatives', derivative, sub, ses, 'nback', '{sub}_{ses}_task-nback_run-1+2_space-MNI152NLin2009cAsym_res-2_desc-mask.nii.gz'.format(sub=sub, ses=ses)))
...
for tmp_run_brik_fn in nback_briks:
    ...
    if len(tmp_brik_fn_list) > 1:
        final_brik_list.append(op.join(bids_dir, 'derivatives', derivative, sub, ses, 'nback', '{sub}_{ses}_task-nback_run-1+2_space-MNI152NLin2009cAsym_res-2_desc-REMLbucket+SPMG+tlrc.HEAD'.format(sub=sub, ses=ses)))
    elif len(tmp_brik_fn_list) == 1:
        final_brik_list.append(tmp_run_brik_fn)
    fd = [pd.read_csv(op.join(bids_dir, 'derivatives', 'fmriprep-20.2.1', 'fmriprep', sub, 'ses-baselineYear1Arm1', 'func', '{}_desc-confounds_timeseries.tsv'.format(i.split('/')[-1].split('_space-')[0])), sep='\t')['framewise_displacement'].mean() for i in tmp_brik_fn_list]
...
label_bucket_dict = {'two_back_gt_zero_back':47, 'negative_faces_gt_neutral_faces':53, 'positive_faces_gt_neutral_faces':56} #'zero_back':41, 'two_back':44, 'faces_gt_places': 50,
for label in label_bucket_dict.keys():
    ...
    with open(op.join(out_dir, 'nback-group-{label}'.format(label=label), 'args_file.txt'), 'w') as fo:
        fo.write('-setA Group\n')
    ...
    with open(op.join(out_dir, 'nback-group-{label}'.format(label=label), 'covariates_file.txt'), 'w') as fo:
    ...
    cmd = '3dttest++ -prefix {bucket_fn} -mask {mask_fn} -Covariates {covariates_file} -Clustsim -@ < {args_file}'.format(bucket_fn=cluster_bucket_fn,
                                                                                                                          mask_fn=mask_fn,
                                                                                                                          covariates_file=op.join(out_dir, 'nback-group-{label}'.format(label=label), 'covariates_file.txt'),
                                                                                                                          args_file=op.join(out_dir, 'nback-group-{label}'.format(label=label), 'args_file.txt'))
    #os.system(cmd)

    #now unpaired ttest
    os.makedirs(op.join(bids_dir, 'derivatives', derivative, 'nback-group-comparison', 'nback-group-{label}'.format(label=label)), exist_ok=True)
    os.chdir(op.join(bids_dir, 'derivatives', derivative, 'nback-group-comparison', 'nback-group-{label}'.format(label=label)))
    cluster_bucket_fn = 'nback-{}'.format(label)
    ...
    with open(op.join(bids_dir, 'derivatives', derivative, 'nback-group-comparison', 'nback-group-{label}'.format(label=label), 'args_file.txt'), 'w') as fo:
        fo.write('-setA Bicult {setA}\n -setB Detached {setB}\n'.format(setA=" ".join(setA), setB=" ".join(setB)))

    with open(op.join(bids_dir, 'derivatives', derivative, 'nback-group-comparison', 'nback-group-{label}'.format(label=label), 'covariates_file.txt'), 'w') as fo:
        fo.write('subject age_p age_c site FD education income nativity_p nativity_c gender_p gender_c\n')
        for brik in bucket_list:
            tmp_sub = op.basename(brik).split('_')[0]
            with open(op.join(op.dirname(brik), 'fd_mean.txt'), 'r') as f:
    ...
    cmd = '3dttest++ -prefix {bucket_fn} -mask {mask_fn} -Covariates {covariates_file} -Clustsim -@ < {args_file}'.format(bucket_fn=cluster_bucket_fn,
                                                                                                                          mask_fn=mask_fn,
                                                                                                                          covariates_file=op.join(bids_dir, 'derivatives', derivative, 'nback-group-comparison', 'nback-group-{label}'.format(label=label), 'covariates_file.txt'),
                                                                                                                          args_file=op.join(bids_dir, 'derivatives', derivative, 'nback-group-comparison', 'nback-group-{label}'.format(label=label), 'args_file.txt'))
    os.system(cmd)
    os.chdir(curr_dir)
