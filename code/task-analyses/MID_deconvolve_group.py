...
if not op.isfile(op.join(out_dir, "group_bold.tsv")):
    ...
    cmd = "singularity run --cleanenv \
               ...
               group --no-sub --task-id MID".format(
...
for qc_metric in qc_metrics:
    ...
    if qc_metric in ["efc", "fd_mean", "gsr_x", "gsr_y"]:
        runs_exclude_df = runs_exclude_df.append(
            mriqc_group_MID_df.loc[mriqc_group_MID_df[qc_metric].values > q95], ignore_index=True
        )
    elif qc_metric in ["snr", "tsnr"]:
        runs_exclude_df = runs_exclude_df.append(
            mriqc_group_MID_df.loc[mriqc_group_MID_df[qc_metric].values < q05], ignore_index=True
        )
...
MID_masks = sorted(
    glob(
        op.join(
            bids_dir, "derivatives", derivative, "sub-*", "ses-*", "MID", "*MID*desc-mask.nii.gz"
        )
    )
)
MID_briks = sorted(
    glob(op.join(bids_dir, "derivatives", derivative, "sub-*", "ses-*", "MID", "*MID*tlrc*HEAD"))
)
...
for i, row in runs_exclude_df.iterrows():
    ...
    tmp_run_mask_fn = op.join(
        ...
        ses,
        "MID",
        "{}_space-MNI152NLin2009cAsym_desc-mask.nii.gz".format(tmp_run),
    )
    ...
    if len(tmp_mask_fn_list) > 1:
        final_mask_list.append(
            op.join(
                ...
                ses,
                "MID",
                "{sub}_{ses}_task-MID_run-1+2_space-MNI152NLin2009cAsym_desc-mask.nii.gz".format(
                    sub=sub, ses=ses
                ),
            )
        )
    ...
    if len(tmp_brik_fn_list) > 1:
        final_brik_list.append(
            op.join(
                ...
                ses,
                "MID",
                "{sub}_{ses}_task-MID_run-1+2_space-MNI152NLin2009cAsym_desc-REMLbucket+SPMG+tlrc.HEAD".format(
                    sub=sub, ses=ses
                ),
            )
        )
...
label_bucket_dict = {
    "large_reward_gt_neutral_antic": 76,
    "small_reward_gt_neutral_antic": 79,
    "large_loss_gt_neutral_antic": 82,
    "small_loss_gt_neutral_antic": 85,
    "reward_pos_feedback_gt_reward_neg_feedback": 88,
    "loss_pos_feedback_gt_loss_neg_feedback": 91,
}
for label in label_bucket_dict.keys():
    ...
    cmd = "3dttest++ -prefix {bucket_fn} -mask {mask_fn} -setA {bucket_list}".format(
        bucket_fn=bucket_fn, mask_fn=mask_fn, bucket_list=" ".join(bucket_list)
    )
    os.system(cmd)
