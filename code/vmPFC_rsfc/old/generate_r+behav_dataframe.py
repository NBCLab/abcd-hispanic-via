...
derivative_directory = op.join(
    project_directory,
    "dset/derivatives/fmriprep_post-process_Cluster1+Cluster2+Cluster3+Cluster4+Cluster5+Cluster6",
)
...
covar = pd.read_csv(
    op.join(derivative_directory, analyses_directory, "covariates_file.txt"), sep="\s+"
)

# first add the summary scores
# parent prosocial behavior and family conflict subscale
sscep = pd.read_csv(
    op.join(project_directory, "code", "abcdclinicaldata05032021", "abcd_sscep01.txt"), sep="\t"
)
...
for i, row in covar.iterrows():
    tmp_df = sscep[
        (sscep["subjectkey"] == "NDAR_{}".format(row["subject"].split("sub-NDAR")[1]))
        & (sscep["eventname"] == "baseline_year_1_arm_1")
    ]
...
# now youth
sscey = pd.read_csv(
    op.join(project_directory, "code", "abcdclinicaldata05032021", "abcd_sscey01.txt"), sep="\t"
)
# prosocial variable: psb_p_ss_mean
psb_y = []
for i, row in covar.iterrows():
    tmp_df = sscey[
        (sscey["subjectkey"] == "NDAR_{}".format(row["subject"].split("sub-NDAR")[1]))
        & (sscey["eventname"] == "baseline_year_1_arm_1")
    ]
...
# cbcl stuff
cbcl = pd.read_csv(
    op.join(project_directory, "code", "abcdclinicaldata05032021", "abcd_cbcls01.txt"), sep="\t"
)
# cbcl variables
...
for i, row in covar.iterrows():
    tmp_df = cbcl[
        (cbcl["subjectkey"] == "NDAR_{}".format(row["subject"].split("sub-NDAR")[1]))
        & (cbcl["eventname"] == "baseline_year_1_arm_1")
    ]
...
roi_mask_fn = op.join(
    derivative_directory, analyses_directory, "GroupCProb1-GroupCProb2_Zscr_cluster-1_mask.nii.gz"
)

roi_values = []
for i, row in covar.iterrows():
    tmp_img_fn = op.join(
        derivative_directory, row["subject"], "ses-baselineYear1Arm1", "rest-Cluster1+tlrc.BRIK"
    )
...
covar.to_csv(
    op.join(derivative_directory, analyses_directory, "roi_values+behavior_cluster-1.csv"),
    sep="\t",
)
...
plt.savefig(
    op.join(derivative_directory, analyses_directory, "psb_p_correlation.png"),
    format="png",
    dpi=150,
)
plt.close()
...
plt.savefig(
    op.join(derivative_directory, analyses_directory, "fes_p_ss_fc_correlation.png"),
    format="png",
    dpi=150,
)
plt.close()
...
plt.savefig(
    op.join(derivative_directory, analyses_directory, "psb_y_correlation.png"),
    format="png",
    dpi=150,
)
plt.close()
